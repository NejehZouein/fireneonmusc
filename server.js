    ...
    switch (comando) {
        ...
        if (message.member.voiceChannel) {
          if (mess === prefix + "play")
            message.channel.send(
              ":x: **`No escribiste el nombre de una canción`**"
            );
          else {
            ...
            if (isURL(args.toLowerCase())) {
              ...
              // Youtube
                ...
                message.channel.send(
                  ":x: **`No se encontró ninguna canción con ese link`**"
                );
            } // Si no
      ...
      case prefix + "skip":
        if (guilds[message.guild.id].queue[0] !== undefined) {
          message.channel.send(
            ":white_check_mark: **`La canción fue saltada exitosamente`**"
          );
    ...
    if (json.tracks)
      ...
      if (guilds[message.guild.id].queue.length > 0) {
        ...
        if (guilds[message.guild.id].queue.indexOf(id) > -1)
          ...
          message.channel.send(
            ":warning: **`Esa canción ya está en cola, espera a que acabe para escucharla otra vez`**"
          );
...
async function buscar_video(args, message) {
  ...
  var json = JSON.parse(respuesta);
  if (!json.items[0])
    message.channel.send(":x: **`No se encontró ninguna canción`**");
  else {
...
function agregar_a_cola(message, id, url, titulo, duracion) {
  message.channel.send(
    "📢 Has añadido una canción a la cola: ```🎵: " +
      titulo +
      "\n⏲️: [" +
      duracion +
      "]\n📽️: " +
